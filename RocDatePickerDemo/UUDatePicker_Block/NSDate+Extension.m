...
- (NSUInteger)previousMonthDaysForPreviousDate{
    return [[self previousMonthDate] daysInMonth];
}
 ...
 *  是否是同个月
 *
 *  @return Return if self is month
 */
...
- (NSDate *)offsetMinute:(int)minute {
    return [NSDate offsetMinute:minute fromDate:self];
}
...
+ (NSDate *)offsetMinute:(int)numMinute fromDate:(NSDate *)fromDate {
    ...
    NSDateComponents *offsetComponents = [[NSDateComponents alloc] init];
    [offsetComponents setMinute:numMinute];
